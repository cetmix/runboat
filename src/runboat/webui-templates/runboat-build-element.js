  ...
  render() {
    ...
    return html`
      <div class="build-card build-status-${this.build.status}">
        ...
        <p>
          <a href="${this.build.repo_target_branch_link}"
            >${this.build.commit_info?.repo}
            ${this.build.commit_info?.target_branch}</a
          >
          ${this.build.commit_info?.pr
            ? html`PR
                <a href="${this.build.repo_pr_link}"
                  >${this.build.commit_info?.pr}</a
                >`
            : ""}
          ${this.build.commit_info?.git_commit
            ? html`(<a href="${this.build.repo_commit_link}"
                  >${this.build.commit_info?.git_commit.substring(0, 8)}</a
                >)`
            : ""}
        ...
        <p>
          ...
          ${this.build.status
            ? html`⦙ 🗒
                <a href="/api/v1/builds/${this.build.name}/init-log"
                  >init log</a
                >`
            : ""}
          ...
          ${this.build.status == "started"
            ? html`⦙ 🚪
                <a href="${this.build.deploy_link}" title="Odoo">live</a>
                <a href="${this.build.deploy_link_mailhog}" title="Mailhog"
                  >✉</a
                >`
            : ""}
        </p>
        <p>
          <button
            ...
            ?disabled="${this.build.status != "stopped" ||
          ...
          <button
            ...
            ?disabled="${this.build.status != "started" ||
            this.build.status == RunboatBuildElement.clickedStatus}"
          >
            stop
          </button>
          <button
            ...
            ?disabled="${this.build.status ==
            RunboatBuildElement.clickedStatus}"
          >
            reset
          </button>
        </p>
